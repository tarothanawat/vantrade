...
const CHART_H       = 320;
const PADDING       = 32;
const LABEL_W       = 72;   // right-side Y-axis label area
const TOTAL_SVG_W   = CHART_W + LABEL_W;  // 1032

const X_AXIS_H      = 28;   // bottom strip for time labels
const VOLUME_H      = 52;   // volume histogram height
const PRICE_BOTTOM  = CHART_H - X_AXIS_H - VOLUME_H;  // 240
const VOL_TOP       = PRICE_BOTTOM;        // 240
const VOL_BOTTOM    = CHART_H - X_AXIS_H; // 292
const AXIS_RIGHT    = CHART_W - PADDING;  // 928
...
export default function InteractiveMarketChart({
  ...
  // ── Pan / zoom state ───────────────────────────────────────────────────
  const [scrollBack,  setScrollBack]  = useState(0);     // bars scrolled from the right end
  ...
  // Ref for wheel handler (avoids stale closure without passive-event workaround)
  const svgRef   = useRef<SVGSVGElement>(null);
  const wheelRef = useRef({ scrollBack: 0, clampedVisible: 0, startIdx: 0, barsLength: 0 });
  const rafRef        = useRef<number | null>(null);  // throttle wheel: one commit per animation frame
  const hoverRafRef   = useRef<number | null>(null);  // throttle pointer-move crosshair updates
  ...
  useEffect(() => {
    ...
    if (key !== prevFirstBarKeyRef.current) {
      setScrollBack(0);
      // Show ~70% of bars by default so the user can immediately drag right
      // to reveal the remaining ~30% of older bars. Scroll wheel zooms further.
      setVisibleCount(
        bars.length > MIN_BARS_VISIBLE ? Math.round(bars.length * 0.7) : null,
      );
  ...
  // Keep wheel ref current (runs synchronously in render, safe for the effect)
  wheelRef.current = { scrollBack, clampedVisible, startIdx, barsLength: bars.length };

  // ── Non-passive wheel listener (React 18 makes onWheel passive by default) ─
  useEffect(() => {
    ...
    const handler = (e: WheelEvent) => {
      ...
      // Throttle: accumulate delta across frames, commit once per animation frame.
      // Without this, the wheel fires at 60+ fps and queues hundreds of setState
      // calls before React can commit a single render — causing visible freezes.
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        // Scroll down = zoom out (more bars), scroll up = zoom in (fewer bars)
        const factor = e.deltaY > 0 ? 1.15 : 0.87;
        const newVis = clamp(Math.round(clampedVisible * factor), MIN_BARS_VISIBLE, barsLength);
        if (newVis === clampedVisible) return;

        // Keep the bar under the cursor at the same horizontal fraction
  ...
  }, []); // uses wheelRef/rafRef — no stale-closure risk

  // ── Chart geometry (depends on visible slice, not full bars) ─────────────
  const chart = useMemo(() => {
    ...
    const margin = rawRange * 0.06;   // 6% breathing room so wicks don't graze the edges
  ...
  // Latest price from the FULL bars array so the badge stays current even when panned
  ...
  const activeIndex = hoveredIndex !== null
    ? clamp(hoveredIndex, 0, visibleBars.length - 1)
  ...
  const horizontalY = hoveredSvgY !== null
    ? clamp(hoveredSvgY, PADDING, PRICE_BOTTOM)
    : dotY;
  ...
  function onPointerMove(e: React.PointerEvent<SVGSVGElement>) {
    ...
    if (isDragging) {
      // Pan: drag right → reveal older data (scrollBack increases).
      // Drag updates are already tied to pointer capture so no extra throttle needed.
      ...
      const maxSB  = bars.length - clampedVisible;
      setScrollBack(clamp(dragStartScrollBackRef.current + delta, 0, maxSB));
      return;
    }

    // Crosshair — throttle to one update per animation frame to avoid
    // queuing two setState calls on every pixel of mouse movement.
    ...
    hoverRafRef.current = requestAnimationFrame(() => {
      ...
      const idx  = Math.round(
        ((svgX - PADDING) / Math.max(CHART_W - PADDING * 2, 1)) * (visibleBars.length - 1),
      );
  ...
  function onRsiMouseMove(e: React.MouseEvent<SVGSVGElement>) {
    ...
    const idx  = Math.round(
      ((svgX - sc.RSI_P) / Math.max(sc.RSI_W - sc.RSI_P * 2, 1)) * (visibleBars.length - 1),
    );
    setHoveredIndex(clamp(idx, 0, visibleBars.length - 1));
    setHoveredSvgY(null);
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between text-sm">
        ...
        <p className="text-gray-400">
          Last:{' '}
          <span className="text-gray-200">{formatTimestamp(sc.latest.timestamp)}</span>
        </p>
      </div>
          ...
          >
            ...
            {sc.yAxisLevels.map((lv, i) => (
              <g key={i}>
                ...
                <text x={AXIS_RIGHT + 8} y={lv.y + 4} fill="#6b7280" fontSize="10" textAnchor="start">
            ...
            {showGridlines && sc.yAxisLevels.map((lv, i) => (
              <line key={i}
                x1={PADDING} y1={lv.y} x2={AXIS_RIGHT} y2={lv.y}
            ...
            {showVolume && visibleBars.map((bar, i) => {
              ...
              const top    = sc.toVolY(bar.volume);
              const h      = Math.max(VOL_BOTTOM - top, 1);
              return (
                <rect key={i}
                  x={x - sc.candleWidth / 2} y={top}
                  width={sc.candleWidth} height={h}
                  fill={isBull ? '#10b981' : '#ef4444'} opacity="0.4"
                />
              );
            })}
            {showVolume && (
            ...
            {/* ── Price series ── */}
            {chartMode === 'line' ? (
              <path d={sc.linePath} fill="none" stroke="#818cf8" strokeWidth="2.5"
                strokeLinejoin="round" strokeLinecap="round" />
            ) : (
              visibleBars.map((bar, i) => {
                const x         = sc.toX(i);
                const openY     = sc.toY(bar.open);
                const closeY    = sc.toY(bar.close);
                const highY     = sc.toY(bar.high);
                const lowY      = sc.toY(bar.low);
                ...
                return (
                  <g key={`${bar.timestamp.toISOString()}-${i}`}>
                    <line x1={x} y1={highY} x2={x} y2={lowY}
                      stroke={isBull ? '#34d399' : '#fb7185'} strokeWidth="1.2" />
                    <rect
                      x={x - sc.candleWidth / 2} y={bodyTop}
                      width={sc.candleWidth} height={bodyH}
                      fill={isBull ? '#10b981' : '#ef4444'} opacity="0.9"
                    />
                  </g>
                );
              })
            ...
            <line
              x1={PADDING} y1={currentPriceY} x2={AXIS_RIGHT} y2={currentPriceY}
              stroke={currentPriceColor} strokeWidth="1" strokeDasharray="4 3" opacity="0.85"
            />
            <rect x={AXIS_RIGHT + 1} y={currentPriceY - 10} width={LABEL_W - 2} height={20}
              fill={currentPriceColor} rx="3" />
            <text x={AXIS_RIGHT + LABEL_W / 2} y={currentPriceY + 4}
            ...
            {!isDragging && (
              <>
                <line x1={crosshairX} y1={PADDING} x2={crosshairX} y2={PRICE_BOTTOM}
                  stroke="#94a3b8" strokeWidth="1" strokeDasharray="4 4" opacity="0.9" />
                <line x1={PADDING} y1={horizontalY} x2={AXIS_RIGHT} y2={horizontalY}
                  stroke="#64748b" strokeWidth="1" strokeDasharray="4 4" opacity="0.8" />
                <circle cx={crosshairX} cy={dotY} r="3.5" fill="#e2e8f0" />

                {/* Cursor price badge on Y-axis */}
                {hoveredIndex !== null && (
                  <>
                    <rect x={AXIS_RIGHT + 1} y={horizontalY - 10} width={LABEL_W - 2} height={20}
                      fill="#1e293b" stroke="#475569" strokeWidth="1" rx="3" />
                    <text x={AXIS_RIGHT + LABEL_W / 2} y={horizontalY + 4}
            ...
            {/* ── OHLCV overlay — horizontal row top-left ── */}
            {showOhlcvOverlay && (() => {
              const stats = [
                { label: 'O',   value: formatPrice(activeBar.open) },
                { label: 'H',   value: formatPrice(activeBar.high) },
                { label: 'L',   value: formatPrice(activeBar.low) },
                { label: 'C',   value: formatPrice(activeBar.close) },
                { label: 'Vol', value: activeBar.volume.toFixed(0) },
                { label: 'RSI', value: activeRsi === null ? 'N/A' : activeRsi.toFixed(1) },
              ];
              const oy = PADDING + 14;
              return (
                <g>
            ...
            {/* ── X-axis static time labels ── */}
            {showXAxis && sc.xAxisLabels.map(({ x, label }, i) => (
              <text key={i} x={x} y={CHART_H - X_AXIS_H + 14}
                fill="#6b7280" fontSize="9" textAnchor="middle">
                {label}
              </text>
            ))}

            {/* ── Crosshair timestamp badge on X-axis ── */}
            {hoveredIndex !== null && showXAxis && !isDragging && (
              <>
                <rect x={crosshairX - 28} y={CHART_H - X_AXIS_H + 2}
                  width={56} height={16} fill="#1e293b" stroke="#475569" strokeWidth="1" rx="3" />
                <text x={crosshairX} y={CHART_H - X_AXIS_H + 13}
                  fill="#e2e8f0" fontSize="9" textAnchor="middle">
                  {formatXLabel(activeBar.timestamp, timeframe)}
                </text>
              </>
            )}

            {/* ── Scroll position indicator (shown only when not at the right edge) ── */}
            {scrollBack > 0 && (
              <text x={AXIS_RIGHT - 4} y={PADDING + 12} fill="#64748b" fontSize="9" textAnchor="end">
                {scrollBack} bars back · dbl-click to reset
              </text>
            )}
          </svg>
        </div>
